package com.yh.bannerlibary.recyclerviewpager;
...
import com.yh.bannerlibary.BuildConfig;
...
public class RecyclerViewPager extends RecyclerView {
    ...
    public void onScrolled(int dx, int dy) {
        ...
        if (mOnPageChangedListeners != null) {
            for (OnPageChangedListener onPageChangedListener : mOnPageChangedListeners) {
                if (onPageChangedListener != null) {
                    onPageChangedListener.onPageScrolled(mPositionBeforeScroll, getScrollX(), dx);
                }
            }
        }
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN && getLayoutManager() != null) {
            mPositionOnTouchDown = getLayoutManager().canScrollHorizontally() ? ViewUtils.getCenterXChildPosition(
                    this) : ViewUtils.getCenterYChildPosition(this);
            if (DEBUG) {
        ...
        if (isInertia) {
            ...
            switch (MotionEvent.ACTION_MASK & e.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    float lastDistance = (float) Math.sqrt(
                            touchStartPoint.x * touchStartPoint.x + touchStartPoint.y * touchStartPoint.y);
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_DRAGGING) {
            mNeedAdjust = true;
            mCurView = getLayoutManager().canScrollHorizontally() ? ViewUtils.getCenterXChild(this) : ViewUtils.getCenterYChild(
                    this);
            if (mCurView != null) {
            ...
            if (mNeedAdjust) {
                int targetPosition = getLayoutManager().canScrollHorizontally() ? ViewUtils.getCenterXChildPosition(
                        this) : ViewUtils.getCenterYChildPosition(this);
                if (mCurView != null) {
    ...
    protected RecyclerViewPagerAdapter ensureRecyclerViewPagerAdapter(Adapter adapter) {
        return (adapter instanceof RecyclerViewPagerAdapter) ? (RecyclerViewPagerAdapter) adapter : new RecyclerViewPagerAdapter(
                this, adapter);
